...
```
we can see there are 4 attributes that are positively correlated to the class label (Dataset), which are:
Gender, Albumin_and_Globulin_Ratio, Total_Protiens, Albumin. these attributes will be good for splitting the tree in the classification.

# discretize the age attribute 
we decided to perform this step to make classification simpler by assigning all the patients to an appropriate interval. the bew intervals are as follows : 4-12, 13-20, 21-29, 30-39, 40-49, 50-59, 60-69, 70-79, 80-90.
```{r}
...
```{r}
 plot(prdataset$Alamine_Aminotransferase, 
     prdataset$Aspartate_Aminotransferase, 
...
# Create a bar plot
```{r}
barplot(table(prdataset$Gender),
        ylab = "Frequency",
        xlab = "Gender",
        col =" blue")
```
...
```{r}
class_label <- factor(prdataset$Dataset)

library(ggplot2)

  ggplot(prdataset, aes(x = Age, fill = class_label)) +
  geom_density(alpha = 0.6) +
  labs(x = "Age", y = "Density", fill = "Liver Disease") +
  ggtitle("Distribution of Age by Liver Disease") +
  theme_minimal()
 
```
from the plot we can see the same age groups can be found in both patients with liver disease(1) and patients without liver disease(2), which indicates the variety in ages. what is concerning though is many young kids are diagnosed with liver disease, we thought its highly unlikely for kids to be diagnosed with such diseases but as it turns out this is not the case, in fact less older patients are diagnosed with liver disease.


# carete a scatter plot for Total_Bilirubin and class label (dataset)
```{r}
plot(prdataset$Dataset, 
     prdataset$Total_Bilirubin, 
     xlab = "dataset", 
...
```{r}
plot(prdataset$Dataset, 
     prdataset$Alkaline_Phosphotase, 
     xlab = "dataset", 
...
```{r}
plot(prdataset$Dataset, 
     prdataset$Alamine_Aminotransferase, 
     xlab = "dataset", 
