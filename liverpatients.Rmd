...
Alamine_Aminotransferase,Aspartate_Aminotransferase attributes they seem to be correlated and their correlation is high and we will make sure of that with calcuations.   
...
# check correlation
```{r}
cor(dataset$Aspartate_Aminotransferase,dataset$Alamine_Aminotransferase)
cor(dataset$Total_Bilirubin,dataset$Direct_Bilirubin)
cor(dataset$Total_Protiens,dataset$Albumin)
cor(dataset$Albumin_and_Globulin_Ratio,dataset$Albumin)
```
these are all highly correlated attributes, therefore, no need for feature selection.
